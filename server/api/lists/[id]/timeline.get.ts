...
export default defineEventHandler(async (event): Promise<TimelinePageResponse> => {
  ...
  const endpoint = new URL(`/api/v1/timelines/list/${id}`, serverOrigin)
  ...
  if (!response.ok) {
    ...
    throw createError({ statusCode: response.status, statusMessage: detail || 'Failed to fetch list timeline.' })
  }
