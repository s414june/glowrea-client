...
import { fetchBookmarksPage } from '~/utils/api/bookmarks'
...
export function useBookmarks() {
  ...
  async function loadInitial(): Promise<void> {
    ...
    try {
      const res = await fetchBookmarksPage()
  ...
  async function loadMore(): Promise<void> {
    ...
    loadMoreError.value = null
    try {
      const res = await fetchBookmarksPage(nextMaxId.value)
  ...
  return { items, isInitialLoading, isLoadingMore, initialError, loadMoreError, hasMore, isEmpty, loadInitial, loadMore, retryInitial: loadInitial }
}
