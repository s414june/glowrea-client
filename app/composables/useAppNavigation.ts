...
export type NavItem = {
  key: 'home' | 'notifications' | 'explore' | 'profile' | 'more' | 'compose'
  label: string
  to: string
  icon: 'home' | 'notifications' | 'search' | 'explore' | 'profile' | 'more' | 'compose'
}

const navItems: NavItem[] = [
  { key: 'home', label: '首頁', to: '/timelines', icon: 'home' },
  { key: 'notifications', label: '通知', to: '/notifications', icon: 'notifications' },
  { key: 'explore', label: '探索', to: '/explore', icon: 'search' },
...
const AUTH_DESKTOP_KEYS = ['home', 'notifications', 'explore', 'profile', 'more']
// 未登入桌機側欄：只保留無需驗證的頁籤
const GUEST_DESKTOP_KEYS = ['home', 'explore', 'more']

// 行動版頂部小圖示（more 按鈕由版面層單獨渲染）
const AUTH_MOBILE_TOP_KEYS = ['more']
const GUEST_MOBILE_TOP_KEYS = ['more']

// 行動版底部列
const AUTH_MOBILE_BOTTOM_ORDER = ['home', 'notifications', 'compose', 'explore', 'profile']
const GUEST_MOBILE_BOTTOM_ORDER = ['home', 'explore']
