...
- 登出後首頁 `/timelines` 的 ComingSoon 佔位改為顯示本站時間軸（同步升級）
...
7. 登出狀態下的 `/timelines` 自動改顯示本站時間軸，不需跳轉路由
...
`/timelines` 為 parent layout，頁籤列永遠可見，子頁面依路由切換：

- `/timelines`（index）→ 追蹤時間軸（需登入，未登入顯示 `ComingSoon`）
- `/timelines/messages` → 私訊（需登入）
- `/timelines/:instance/local` → 本站時間軸（訪客可存取）
- `/timelines/federated` → 聯邦時間軸（`ComingSoon` placeholder）
...
登出後 `/timelines` 顯示頁籤列，訪客可切換至 `本站` 或 `聯邦` tab；追蹤、私訊 tab 不渲染（需登入）。
